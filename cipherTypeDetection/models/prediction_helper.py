...
def split_ciphertext_to_model_input_length(model, architecture, ciphertexts):
    ...
    def pad_and_split_single_ciphertext(ciphertext):
        ...
        return [ciphertext[input_length*j:input_length*(j+1)] for j in range(len(ciphertext) // input_length)]
...
def predict_lstm(model, ciphertext, batch_size, backend):
    split_ciphertext, _ = split_ciphertext_to_model_input_length(model, "LSTM", ciphertext)

    predictions = []
    if backend == Backend.KERAS:
        for ct in split_ciphertext:
            predictions.append(model.predict(tf.convert_to_tensor([ct]), batch_size, verbose=0))
    elif backend == Backend.PYTORCH:
        for ct in split_ciphertext:
...
def predict_cnn(model, ciphertext, batch_size):
    split_ciphertext, input_length = split_ciphertext_to_model_input_length(model, "CNN", ciphertext)
...
def predict_transformer(model, ciphertext, batch_size):
    split_ciphertext, _ = split_ciphertext_to_model_input_length(model, "Transformer", ciphertext)
